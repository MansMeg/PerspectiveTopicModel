...
#'
#' @export
collapsed_sampler <- function(state, priors, params){
  # Assertions
  ...
  ### Setup sampler
  results <- collapsed_sampler_cpp(state = state, count_matrices = count_matrices, priors = priors, constants = constants)
  ...
  for (step in (params$start_iter + 1):params$gibbs_iter){
    results <- collapsed_sampler_cpp(state = results$state, count_matrices = results$count_matrices, priors = priors, constants = constants)
